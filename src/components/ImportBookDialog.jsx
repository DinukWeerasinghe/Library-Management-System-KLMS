...
import { Download, Upload, AlertCircle, CheckCircle, XCircle, ArrowRight, Save } from 'lucide-react';
...
export function ImportBookDialog({ onClose, onImportComplete }) {
    ...
    const handleExecute = async () => {
        setLoading(true);
        try {
            // Filter out invalid rows before sending? Backend also ignores them but saving bandwidth is good.
            // Sending entire rows array for simplicity as structure expects it.
                    ...
                    <tbody>
                        {previewData.rows.map((row) => (
                            <tr key={row.id} className={`row-${row.status.toLowerCase()}`}>
                                ...
                                <td>
                                    {row.data.title || <i className="muted">Missing</i>}
        ...
        <div className="import-modal-overlay" onClick={onClose}>
            <div className="import-modal" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3>Bulk Book Import {step === 2 && '- Preview'} {step === 3 && '- Results'}</h3>
