...
import { Download, Upload, AlertCircle, CheckCircle, XCircle, Save } from 'lucide-react';
import { DialogService } from '../services/DialogService';

export function ImportMemberDialog({ onClose, onImportComplete }) {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const res = await window.klms.members.downloadTemplate();
            if (res.success) {
                DialogService.showSuccess('Member template downloaded successfully.');
            } else if (res.error) {
    ...
    const handlePreview = async () => {
        setLoading(true);
        try {
            const res = await window.klms.members.previewImport();
            if (res.canceled) {
                setLoading(false);
                return;
            }
            if (res.success) {
                ...
                DialogService.showError(res.error || 'Preview failed.');
            }
        } catch (err) {
            console.error(err);
    ...
    const handleExecute = async () => {
        setLoading(true);
        try {
            const res = await window.klms.members.executeImport(previewData.rows);
            if (res.success) {
    ...
    const renderStep1 = () => (
        <div className="modal-body step-1">
            <div className="section">
                ...
                <p className="muted">Download the CSV template with required member fields.</p>
                <button onClick={handleDownloadTemplate} className="btn-secondary">
                    <Download size={16} /> Download Member Template
                </button>
            </div>
            <div className="divider"></div>
            <div className="section">
                <h4>Step 2: Upload Member CSV</h4>
                <p className="muted">Select your member CSV file to preview the import.</p>
                <button onClick={handlePreview} className="btn-primary" disabled={loading}>
                    {loading ? 'Analyzing...' : (
                        <>
                            <Upload size={16} /> Select File & Preview
                        </>
                    )}
                </button>
            </div>
        ...
        <div className="modal-body step-2">
            <div className="summary-header">
                <div className="stat-badge new">
                    <span className="count">{previewData.summary.new}</span>
                    <span className="label">New Members</span>
                </div>
                <div className="stat-badge merge">
                    <span className="count">{previewData.summary.merge}</span>
                    <span className="label">Duplicate/Existing</span>
                </div>
                <div className="stat-badge invalid">
                    <span className="count">{previewData.summary.invalid}</span>
                            ...
                            <tr key={row.id} className={`row-${row.status.toLowerCase()}`}>
                                <td>
                                    <span className={`status-pill ${row.status.toLowerCase()}`}>
                                        {row.status === 'MERGE' ? 'EXISTING' : row.status}
                                    </span>
                                </td>
                                <td>
                                    {row.data.name || <i className="muted">Missing</i>}
                                    {row.message && <div className="row-msg">{row.message}</div>}
                                </td>
                                <td>{row.data.member_type}</td>
                                <td>{row.data.member_code}</td>
            ...
            <div className="modal-actions">
                ...
                <button onClick={handleExecute} className="btn-primary" disabled={loading || previewData.summary.new === 0}>
                    {loading ? 'Importing...' : (
                        <>
                            <Save size={16} /> Import New Members
                        </>
                    )}
                </button>
            </div>
    ...
    const renderStep3 = () => (
        <div className="modal-body step-3">
                    ...
                    <table>
                        ...
                        <tbody>
                            {result.errors.map((err, idx) => (
                                <tr key={idx}><td>{err.row}</td><td>{err.message}</td></tr>
                            ))}
                        </tbody>
                    </table>
                </div>
        ...
        <div className="import-modal-overlay" onClick={onClose}>
            <div className="import-modal" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3>Bulk Member Import {step === 2 && '- Preview'} {step === 3 && '- Results'}</h3>
            ...
            <style>{`
                ...
                .preview-table-container { border: 1px solid var(--color-border); border-radius: var(--radius); overflow-x: auto; max-height: 400px; }
