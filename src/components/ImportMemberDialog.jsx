...
export function ImportMemberDialog({ onClose, onImportComplete }) {
    ...
    useEffect(() => {
        if (window.klms?.import?.onProgress) {
            unsubRef.current = window.klms.import.onProgress((data) => {
                if (data.type === 'member') setProgress({ done: data.done, total: data.total });
            });
        }
    ...
    const handleDownloadTemplate = async () => {
        try {
            ...
            if (res.success) DialogService.showSuccess('Member template downloaded successfully.');
    ...
    const handleExecute = async () => {
        const total = previewData.rows.filter(r => r.status === 'NEW').length;
    ...
    const renderStep2 = () => (
        <div className="modal-body">
            <div className="summary-header">
                <div className="stat-badge new"><span className="count">{previewData.summary.new}</span><span className="label">New Members</span></div>
                <div className="stat-badge merge"><span className="count">{previewData.summary.merge}</span><span className="label">Duplicate/Existing</span></div>
            ...
            <div className="preview-table-container">
                <table className="preview-table">
                    <thead><tr><th>Status</th><th>Name</th><th>Type</th><th>Code</th><th>Phone</th></tr></thead>
                    <tbody>
                        {previewData.rows.map((row) => (
                            <tr key={row.id} className={`row-${row.status.toLowerCase()}`}>
                                <td><span className={`status-pill ${row.status.toLowerCase()}`}>{row.status === 'MERGE' ? 'EXISTING' : row.status}</span></td>
                                <td>{row.data.name || <i className="muted">Missing</i>}{row.message && <div className="row-msg">{row.message}</div>}</td>
            ...
            <div className="modal-actions">
                ...
                <button onClick={handleExecute} className="btn-primary" disabled={previewData.summary.new === 0}>
    ...
    const renderStep3 = () => (
        <div className="modal-body progress-body">
            ...
            <h4>Importing {progress.total.toLocaleString()} members...</h4>
        ...
        <div className="import-modal-overlay" onClick={isImporting ? undefined : onClose}>
            <div className="import-modal" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h3>Bulk Member Import{step === 2 ? ' - Preview' : step === 3 ? ' - Importing' : step === 4 ? ' - Results' : ''}</h3>
