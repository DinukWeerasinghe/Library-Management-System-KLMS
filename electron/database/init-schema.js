...
function runInit(wrappedDb) {
  wrappedDb.exec(`
    ...
    CREATE TABLE IF NOT EXISTS Book (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, author TEXT, isbn TEXT, category_id INTEGER, external_code TEXT, internal_code TEXT, barcode_path TEXT, published_year INTEGER, price REAL, acquisition_type TEXT CHECK (acquisition_type IN ('DONATION', 'BOUGHT')), total_copies INTEGER NOT NULL DEFAULT 1, available_copies INTEGER NOT NULL DEFAULT 1, batch_id INTEGER, created_at TEXT DEFAULT (datetime('now', 'localtime')), updated_at TEXT DEFAULT (datetime('now', 'localtime')), FOREIGN KEY (category_id) REFERENCES Category(id), FOREIGN KEY (batch_id) REFERENCES ImportBatch(id));
