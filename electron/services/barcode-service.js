...
async function generateBarcode(code, subfolder = 'member-codes') {
    return new Promise((resolve, reject) => {
        ...
        bwipjs.toBuffer({
            ...
            text: memberCode,      // Text to encode
            ...
            if (err) {
                logger.error(`Barcode generation failed for ${memberCode}:`, err);
                reject(err);
            } else {
                fs.writeFile(filePath, png, (fsErr) => {
                    if (fsErr) {
                        logger.error(`Failed to save barcode image for ${memberCode}:`, fsErr);
                        reject(fsErr);
                    } else {
                        logger.info(`Barcode saved for ${memberCode} at ${filePath}`);
